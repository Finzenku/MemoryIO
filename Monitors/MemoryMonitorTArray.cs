...
namespace MemoryIO.Monitors
{
    ...
    public class MemoryMonitorTArray<T> : IMemoryMonitor<MemoryArrayChangedEventArgs<T>>, IDisposable where T : unmanaged
    {
        /// <summary>
        ...
        public void StartMonitoring()
        {
            ...
            while (address != IntPtr.Zero && isMonitoring)
            {
                for (int i = 0; i < arrayLength; i++)
                {
                    ...
                    {
                        OnMemoryChanged(address, MemoryMarshal.Cast<byte, T>(currentData)[0], i);
